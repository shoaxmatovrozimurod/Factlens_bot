...
// Foydalanuvchilar tilini xotirada saqlash
const userLanguages = {};
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'lang_uz') {
    setUserLanguage(userId, 'uz'); // Faylga saqlash
    await bot.answerCallbackQuery(query.id, { text: "🇺🇿 O'zbek tili tanlandi!" });
    
    // Tugmali xabarni o'chirib tashlash (takrorlanib turmasligi uchun)
    ...
    setUserLanguage(userId, 'en'); // Faylga saqlash
...
bot.on('photo', async (msg) => {
  ...
  // Foydalanuvchi tilini aniqlaymiz ('uz' yoki 'en')
  const lang = getUserLanguage(msg.from.id);
  const text = messages[lang];

  // Kutish xabarini tanlangan tilda yuborish
  ...
  try {
    ...
    // Xulosa va natijani tanlangan tilga moslash
